...
#define REF_SENSOR               2
#define CSX_STATUS_REG           SX9306_TCHCMPSTAT_TCHSTAT3_FLAG
#else
...
#define SX9306_NORMAL_TOUCH_CABLE_THRESHOLD	22
...
static void send_event(struct sx9306_p *data, int cnt, u8 state)
{
	...
	if (msm8974_get_cable_type() > 1) {
		buf = SX9306_NORMAL_TOUCH_CABLE_THRESHOLD;
		pr_info("[SX9306]: %s - cable connected %d\n",
...
int sx9306_regulator_on(struct sx9306_p *data, bool onoff)
{
	...
	|| defined(CONFIG_SEC_T8_PROJECT) \
	|| defined(CONFIG_SEC_T10_PROJECT)
int sx9306_power_on(struct sx9306_p *data, bool onoff)
{
	int ret = -1;
#if defined(CONFIG_SEC_MILLET_PROJECT) \
	|| defined(CONFIG_SEC_BERLUTI_PROJECT) \
